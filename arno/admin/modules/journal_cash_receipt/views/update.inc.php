<script>
    ...
    var options = {
        url: function(keyword) {
            return "controllers/getJournalCashReceiptByKeyword.php?keyword="+keyword;
        },
        ...
        getValue: function(element) {
            return element.journal_cash_receipt_code ;
        },

        template: {
            type: "description",
            fields: {
                description: "journal_cash_receipt_name"
            }
        },
     ...
     function add_row(id){
        ...
        $(id).closest('table').children('tbody').append(
            '<tr class="odd gradeX">'+
                ...
                '</td>'+
                '<td><input type="text" class="form-control" name="journal_cash_receipt_list_name[]" value="' + document.getElementById("journal_cash_receipt_name").value + '" /></td>'+
                '<td align="right"><input type="text" class="form-control" style="text-align: right;"  value="0" onchange="val_format(this);" name="journal_cash_receipt_list_debit[]" onclick="edit_credit(this)"  /></td>'+
                '<td align="right"><input type="text" class="form-control" style="text-align: right;" value="0" onchange="val_format(this);" name="journal_cash_receipt_list_credit[]" /></td>'+
                '<td>'+
            ...
            <div class="panel-heading">
                <div class="row">
                    ...
                    <div class="col-md-4" align="right">
                        <?PHP if($previous_id != ""){?>
                        <a class="btn btn-primary" href="?app=journal_cash_receipt&action=update&id=<?php echo $previous_id;?>" > <i class="fa fa-angle-double-left" aria-hidden="true"></i> <?php echo $previous_code;?> </a>
                        <?PHP } ?>

                        <?PHP if($next_id != ""){?>
                        <a class="btn btn-primary" href="?app=journal_cash_receipt&action=update&id=<?php echo $next_id;?>" >  <?php echo $next_code;?> <i class="fa fa-angle-double-right" aria-hidden="true"></i> </a>
                        <?PHP } ?>
                    </div>
                </div>
            </div>
                                ...
                                <?php 
                                    ...
                                    <tr class="odd gradeX">
                                        <td>
                                            ...
                                            <input type="hidden" name="journal_cheque_id[]" value="<?php echo $journal_cash_receipt_lists[$i]['journal_cheque_id']; ?>" /> 
                                            <input type="hidden" name="journal_cheque_pay_id[]" value="<?php echo $journal_cash_receipt_lists[$i]['journal_cheque_pay_id']; ?>" /> 
                                            <input type="hidden" name="journal_invoice_customer_id[]" value="<?php echo $journal_cash_receipt_lists[$i]['journal_invoice_customer_id']; ?>" />
                                            <input type="hidden" name="journal_invoice_supplier_id[]" value="<?php echo $journal_cash_receipt_lists[$i]['journal_invoice_supplier_id']; ?>" /> 
                                            <input type="hidden" name="journal_cash_receipt_list_id[]" value="<?php echo $journal_cash_receipt_lists[$i]['journal_cash_receipt_list_id']; ?>" />

                                            <select  class="form-control select" name="account_id[]" data-live-search="true"  onchange="get_account_type(this);" 
                                                <?PHP if(
                                                    $journal_cash_receipt_lists[$i]['journal_cheque_id'] > 0 ||
                                                    $journal_cash_receipt_lists[$i]['journal_cheque_pay_id'] > 0 ||
                                                    $journal_cash_receipt_lists[$i]['journal_invoice_customer_id'] > 0 ||
                                                    $journal_cash_receipt_lists[$i]['journal_invoice_supplier_id'] > 0 
                                                ){ ?>
                                                disabled
                                                <?PHP } ?>
                                             >
                                        ...
                                        <td align="right"><input type="text" class="form-control" style="text-align: right;" name="journal_cash_receipt_list_debit[]"  onchange="val_format(this);" value="<?php echo number_format($journal_cash_receipt_lists[$i]['journal_cash_receipt_list_debit'],2); ?>"  onclick="edit_credit(this)" 
                                            <?PHP if(
                                                    $journal_cash_receipt_lists[$i]['journal_cheque_id'] > 0 ||
                                                    $journal_cash_receipt_lists[$i]['journal_cheque_pay_id'] > 0 ||
                                                    $journal_cash_receipt_lists[$i]['journal_invoice_customer_id'] > 0 ||
                                            ...
                                            <?PHP } ?>
                                        /></td>
                                        <td align="right"><input type="text" class="form-control" style="text-align: right;" name="journal_cash_receipt_list_credit[]" onchange="val_format(this);" value="<?php echo number_format($journal_cash_receipt_lists[$i]['journal_cash_receipt_list_credit'],2); ?>"  onclick="edit_credit(this)" 
                                            <?PHP if(
                                                    $journal_cash_receipt_lists[$i]['journal_cheque_id'] > 0 ||
                                                    $journal_cash_receipt_lists[$i]['journal_cheque_pay_id'] > 0 ||
                                                    $journal_cash_receipt_lists[$i]['journal_invoice_customer_id'] > 0 ||
