...
function edit_cheque_pay_row(id,journal_id){
    ...
    $.post( "controllers/getChequePayByChequePayID.php", { 'check_pay_id': check_pay_id }, function( data ) { 
        ...
        if(data !== null){
            ...
            $('#bank_id').val();
            $('#bank_branch').val();
            $('#check_pay_date').val($('#journal_purchase_date').val());
            $('#check_pay_total').val(0);
            $('#check_pay_remark').val($('#journal_purchase_name').val());
            ...
            if(row_journal_id == null ){ 
                ...
                $('#tb_journal').children('tbody').append(
                    '<tr class="odd gradeX">'+
                        ...
                        '</td>'+
                        '<td><input type="text" class="form-control" name="journal_purchase_list_name[]" value="' + data.check_pay_remark + '" /></td>'+
                        '<td align="right"><input type="text" class="form-control" style="text-align: right;" onclick="edit_credit(this);" value="0" onchange="val_format(this);" name="journal_purchase_list_debit[]"   readonly/></td>'+
                        '<td align="right"><input type="text" class="form-control" style="text-align: right;" onclick="edit_credit(this);" value="'+ check_pay_total.toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,") +'" onchange="val_format(this);" name="journal_purchase_list_credit[]" readonly/></td>'+
                        '<td>'+
                ...
                $(row_journal_id).closest('tr').children('td').children('input[name="journal_purchase_list_name[]"]').val(data.check_pay_remark);
                $(row_journal_id).closest('tr').children('td').children('input[name="journal_purchase_list_debit[]"]').val('0');
                $(row_journal_id).closest('tr').children('td').children('input[name="journal_purchase_list_credit[]"]').val(check_pay_total.toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,"));
                $('.select').selectpicker();
            }
