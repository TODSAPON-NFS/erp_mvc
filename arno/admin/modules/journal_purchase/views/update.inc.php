<script>
    ...
    function check(){
        ...
        var journal_purchase_id = document.getElementById("journal_purchase_id").value;
        ...
        if(date_check == "1"){
            ...
            document.getElementById("journal_purchase_code").focus();
            return false;
        }else if(journal_check != "" && journal_check != journal_purchase_id){
     ...
     function add_row(id){
        ...
        $(id).closest('table').children('tbody').append(
            '<tr class="odd gradeX">'+
                ...
                '<td align="right"><input type="text" class="form-control" style="text-align: right;" value="0" onchange="val_format(this);" name="journal_purchase_list_credit[]" onclick="edit_credit(this)" /></td>'+
                '<td>'+
    ...
    <div class="col-lg-12">
        <div class="panel panel-default">
            <div class="panel-heading">
                <div class="row">
                    ...
                    <?PHP if($previous_id != ""){?>
                        ...
                        <a class="btn btn-danger" href="print.php?app=report_journal_01&type=id&action=pdf&id=<?php echo $journal_purchase_id;?>" target="_blank" > <i class="fa fa-print" aria-hidden="true"></i> พิมพ์ </a>
            ...
            <div class="panel-body">
                <form  id="form_target" role="form" method="post" onsubmit="return check();" action="index.php?app=journal_special_01&action=edit&id=<?PHP echo $journal_purchase_id; ?>" enctype="multipart/form-data">
                <input id="journal_purchase_id" name="journal_purchase_id" value="<?PHP echo $journal_purchase['journal_purchase_id']; ?>" type="hidden" />
                    <div class="row">
                        <div class="col-lg-4">
                            <div class="form-group">
                                <label>หมายเลขสมุดรายวันซื้อ / Journal Purchase Code <font color="#F00"><b>*</b></font></label>
                                <input id="journal_purchase_code" name="journal_purchase_code" class="form-control" value="<?php echo $journal_purchase['journal_purchase_code'];?>" onchange="check_code(this)" >
                        ...
                        <div class="col-lg-4">
                            <div class="form-group">
                                ...
                                <input type="text" id="journal_purchase_date" name="journal_purchase_date" value="<?php echo $journal_purchase['journal_purchase_date'];?>"  onchange="check_date(this);" class="form-control calendar" readonly/>
                                ...
                                <?php 
                                    ...
                                    <tr class="odd gradeX">
                                        ...
                                        <td>
                                            ...
                                            <input type="hidden" name="journal_cheque_id[]" value="<?php echo $journal_purchase_lists[$i]['journal_cheque_id']; ?>" /> 
                                            <input type="hidden" name="journal_cheque_pay_id[]" value="<?php echo $journal_purchase_lists[$i]['journal_cheque_pay_id']; ?>" /> 
                                            <input type="hidden" name="journal_invoice_customer_id[]" value="<?php echo $journal_purchase_lists[$i]['journal_invoice_customer_id']; ?>" />
                                            <input type="hidden" name="journal_invoice_supplier_id[]" value="<?php echo $journal_purchase_lists[$i]['journal_invoice_supplier_id']; ?>" /> 
                                            <input type="hidden" name="journal_purchase_list_id[]" value="<?php echo $journal_purchase_lists[$i]['journal_purchase_list_id']; ?>" />

                                            <select  class="form-control select" name="account_id[]" data-live-search="true"  onchange="get_account_type(this);" 
                                                <?PHP if(
                                                    $journal_purchase_lists[$i]['journal_cheque_id'] > 0 ||
                                                    $journal_purchase_lists[$i]['journal_cheque_pay_id'] > 0 ||
                                                    $journal_purchase_lists[$i]['journal_invoice_customer_id'] > 0 ||
                                                    $journal_purchase_lists[$i]['journal_invoice_supplier_id'] > 0 
                                                ){ ?>
                                                disabled
                                                <?PHP } ?>
                                             >
                                        ...
                                        <td align="right"><input type="text" class="form-control" style="text-align: right;" name="journal_purchase_list_debit[]"  onchange="val_format(this);" value="<?php echo number_format($journal_purchase_lists[$i]['journal_purchase_list_debit'],2); ?>"  onclick="edit_credit(this)" 
                                            <?PHP if(
                                                    $journal_purchase_lists[$i]['journal_cheque_id'] > 0 ||
                                                    $journal_purchase_lists[$i]['journal_cheque_pay_id'] > 0 ||
                                                    $journal_purchase_lists[$i]['journal_invoice_customer_id'] > 0 ||
                                            ...
                                            <?PHP } ?>
                                        /></td>
                                        <td align="right"><input type="text" class="form-control" style="text-align: right;" name="journal_purchase_list_credit[]" onchange="val_format(this);" value="<?php echo number_format($journal_purchase_lists[$i]['journal_purchase_list_credit'],2); ?>"  onclick="edit_credit(this)" 
                                            <?PHP if(
                                                    $journal_purchase_lists[$i]['journal_cheque_id'] > 0 ||
                                                    $journal_purchase_lists[$i]['journal_cheque_pay_id'] > 0 ||
                                                    $journal_purchase_lists[$i]['journal_invoice_customer_id'] > 0 ||
