    ...
    for($i=$page_index * $lines; $i < count($debtor_reports) && $i < $page_index * $lines + $lines; $i++){
        ...
        $net_total +=  $debtor_reports[$i]['invoice_customer_net_price'];
                ...
                <tr>
                    ...
                    <td align="center" >'.$debtor_reports[$i]['invoice_customer_date'].'</td>
                    <td>'.$debtor_reports[$i]['invoice_customer_code'].'</td> 
                    <td>['.$debtor_reports[$i]['customer_code'].'] '.$debtor_reports[$i]['invoice_customer_name'].' </td>
                    <td>'.$debtor_reports[$i]['invoice_customer_tax'].' </td>
                    <td align="center" >' .$branch_main.'</td>
                    <td align="center" >' .$branch_sub.'</td>
                    <td  align="right" >
                        '.number_format($debtor_reports[$i]['invoice_customer_total_price'],2).'
                    </td>
                    <td  align="right" >'.number_format($debtor_reports[$i]['invoice_customer_vat_price'],2).'</td>
                    <td  align="right" >'.number_format($debtor_reports[$i]['invoice_customer_net_price'],2).'</td> 
                </tr> 
                ';
    }
