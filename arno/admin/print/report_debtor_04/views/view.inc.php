...
for($page_index=0 ; $page_index < $page_max ; $page_index++){
    ...
    <div align="center" style="font-size:14px;color:#00F;"> <b>รายงานใบรับชำระหนี้ </b></div>
    ...
    for($i=$page_index * $lines; $i < count($debtor_reports) && $i < $page_index * $lines + $lines; $i++){
        $branch = (int)$debtor_reports[$i]['finance_debit_branch'];
        if($branch == 0){
            $branch_main = "/";
            $branch_sub = "";
        }else{
        ...
        $total_page +=  $debtor_reports[$i]['finance_debit_total'];  
        $net_page +=  $debtor_reports[$i]['finance_debit_pay']; 

        $total_total +=  $debtor_reports[$i]['finance_debit_total'];  
        $net_total +=  $debtor_reports[$i]['finance_debit_pay']; 

                $html[$page_index] .= ' 
                <tr>
                    <td align="center" >'.($i + 1).'</td>
                    <td align="center" >'.$debtor_reports[$i]['finance_debit_date'].'</td>
                    <td>'.$debtor_reports[$i]['finance_debit_code'].'</td>  
                    <td>['.$debtor_reports[$i]['customer_code'].'] '.$debtor_reports[$i]['finance_debit_name'].' </td>
                    <td>'.$debtor_reports[$i]['finance_debit_tax'].' </td>
                    <td align="center" >' .$branch_main.'</td>
                    <td align="center" >' .$branch_sub.'</td>
                    <td  align="right" >
                        '.number_format($debtor_reports[$i]['finance_debit_total'],2).'
                    </td> 
                    <td  align="right" >
                        '.number_format($debtor_reports[$i]['finance_debit_pay'],2).'
                    </td>  
                </tr> 
                ';
    }
