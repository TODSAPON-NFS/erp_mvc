...
for($page_index=0 ; $page_index < $page_max ; $page_index++){
    ...
    <table width="100%">
        <tr>
            ...
            <b>หน้า</b> : '.($page_index + 1).'/ '.$page_max.'
            </td>
        </tr>
    </table>
    <div align="center" style="font-size:14px;color:#00F;"> <b>รายงาน​วิเคราะห์อายุ​ลูก​หนี้ แบบย่อ </b></div>
    ...
    for($i=$page_index * $lines; $i < count($debtor_reports) && $i < $page_index * $lines + $lines; $i++){
        ...
        $paper_number_total +=  $debtor_reports[$i]['paper_number']; 
        $due_comming_more_than_60_total +=  $debtor_reports[$i]['due_comming_more_than_60'];  
        $due_comming_in_60_total +=  $debtor_reports[$i]['due_comming_in_60'];  
        $due_comming_in_30_total +=  $debtor_reports[$i]['due_comming_in_30'];  
        $over_due_1_to_30_total +=  $debtor_reports[$i]['over_due_1_to_30'];  
        $over_due_31_to_60_total +=  $debtor_reports[$i]['over_due_31_to_60'];  
        $over_due_61_to_90_total +=  $debtor_reports[$i]['over_due_61_to_90'];  
        $over_due_more_than_90_total +=  $debtor_reports[$i]['over_due_more_than_90'];  
        $balance_total +=  $debtor_reports[$i]['balance'];  
                ...
                <tr >
                    ...
                    <td  align="right" >
                        '. number_format($debtor_reports[$i]['paper_number'],0).'
                    </td>
                    <td  align="right" >
                        '. number_format($debtor_reports[$i]['due_comming_more_than_60'],2).'
                    </td> 
                    <td  align="right" >
                        '. number_format($debtor_reports[$i]['due_comming_in_60'],2).'
                    </td>  
                    <td  align="right" >
                        '. number_format($debtor_reports[$i]['due_comming_in_30'],2).'
                    </td>  
                    <td  align="right" >
                        '. number_format($debtor_reports[$i]['over_due_1_to_30'],2).'
                    </td>  
                    <td  align="right" >
                        '. number_format($debtor_reports[$i]['over_due_31_to_60'],2).'
                    </td>  
                    <td  align="right" >
                        '. number_format($debtor_reports[$i]['over_due_61_to_90'],2).'
                    </td> 
                    <td  align="right" >
                        '. number_format($debtor_reports[$i]['over_due_more_than_90'],2).'
                    </td> 
                    <td  align="right" >
                        '. number_format($debtor_reports[$i]['balance'],0).'
                    </td>  
                </tr>
                ';
    }
