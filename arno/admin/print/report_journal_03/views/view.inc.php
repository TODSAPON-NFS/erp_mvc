...
$cr_total = 0;
 ...
 while($i <  $row_max ){
    ...
    <table width="100%" border="0" cellspacing="0">
        <tr>
            <td align="left" > </td>
            <td colspan = "2" align="left" style="font-size:13px;"> RECEIPT VOUCHER </td> 
        </tr>
        <tr>
            <td align="left" ></td>
    ...
    if(count($journal_cash_receipt_lists) > 0 ){
        $journal_row = 1;
    ...
    if(count($check_pays) > 0){
        ...
        for(; $i < count($journal_cash_receipt_lists)   + count($checks) + count($check_pays) ; $i++){
            $ii = $i - count($journal_cash_receipt_lists)  - count($checks)-$check_row ;
