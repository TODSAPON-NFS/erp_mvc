...
class JournalReportModel extends BaseModel{
  ...
  //------------------------- General Journal -------------------------------------------------------------
    ...
    function getJournalAssetsReportBy($date_end = "", $code_start = "", $code_end = "" ,$keyword = ""){
        ...
        $sql =" SELECT account_code , account_name_th ,  MAX(IFNULL(account_debit_begin,0)) ,SUM(IFNULL(tb_journal.journal_debit,0)), MAX(IFNULL(account_credit_begin,0)) ,SUM(IFNULL(tb_journal.journal_credit,0)) , ( MAX(IFNULL(account_debit_begin,0)) + SUM(IFNULL(tb_journal.journal_debit,0)) ) - ( MAX(IFNULL(account_credit_begin,0)) + SUM(IFNULL(tb_journal.journal_credit,0)) ) as account_value , account_level
                FROM tb_account 
                LEFT JOIN  (($sql_general)  
                UNION   ALL  ($sql_purchase) 
                UNION   ALL  ($sql_sale) 
