...
function edit_cheque_row(id,journal_id){
    ...
    $.post( "controllers/getChequeByChequeID.php", { 'check_id': check_id }, function( data ) { 
        if(data !== null){
            ...
            $('#check_id').val('0');
            $('#check_code').val('QR');  
            $('#check_date_write').val($('#journal_general_date').val()); 
            $('#check_date_recieve').val($('#journal_general_date').val());
            $('#cheque_customer_id').val($('#customer_id').val());
            $('#bank_id').val();
            $('#bank_branch').val();
            $('#check_date').val($('#journal_general_date').val());
            $('#check_total').val(0);
            $('#check_remark').val($('#journal_general_name').val());
            ...
            if(row_journal_id == null ){
                ...
                $('#tb_journal').children('tbody').append(
                    '<tr class="odd gradeX">'+
                        ...
                        '</td>'+
                        '<td><input type="text" class="form-control" name="journal_general_list_name[]" value="' + data.check_remark + '" /></td>'+
                        '<td align="right"><input type="text" class="form-control" style="text-align: right;" onclick="edit_credit(this);" onchange="val_format(this);" name="journal_general_list_debit[]" value="'+ check_total.toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,") +'"  readonly/></td>'+
                        '<td align="right"><input type="text" class="form-control" style="text-align: right;" onclick="edit_credit(this);" value="0" onchange="val_format(this);" name="journal_general_list_credit[]" readonly/></td>'+
                        '<td>'+
                ...
                $(row_journal_id).closest('tr').children('td').children('input[name="journal_general_list_name[]"]').val(data.check_remark);
                $(row_journal_id).closest('tr').children('td').children('input[name="journal_general_list_debit[]"]').val(check_total.toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,"));
                $(row_journal_id).closest('tr').children('td').children('input[name="journal_general_list_credit[]"]').val('0');
                $('.select').selectpicker();
            }
