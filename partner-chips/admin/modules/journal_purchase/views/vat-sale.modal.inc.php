...
function edit_invoice_customer_row(id,journal_id){
    ...
    $.post( "controllers/getInvoiceCustomerByID.php", { 'invoice_customer_id': invoice_customer_id }, function( data ) { 
        ...
        if(data !== null){
            ...
            $('#invoice_customer_date').val($('#journal_purchase_date').val());  
            $('#customer_code').val(''); 
            ...
            $('#invoice_customer_description').val($('#journal_purchase_name').val());
            ...
            if(row_journal_id == null){
                ...
                $('#tb_journal').children('tbody').append(
                    '<tr class="odd gradeX">'+
                        ...
                        '</td>'+
                        '<td><input type="text" class="form-control" name="journal_purchase_list_name[]" value="' + data.invoice_customer_description + '" /></td>'+
                        '<td align="right"><input type="text" class="form-control" style="text-align: right;" onclick="edit_credit(this);"  onchange="val_format(this);" name="journal_purchase_list_debit[]" value="'+ invoice_customer_vat_price_debit.toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,") +'"  readonly/></td>'+
                        '<td align="right"><input type="text" class="form-control" style="text-align: right;" onclick="edit_credit(this);"  onchange="val_format(this);" name="journal_purchase_list_credit[]" value="'+ invoice_customer_vat_price_credit.toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,") +'" readonly/></td>'+
                        '<td>'+
                ...
                $(row_journal_id).closest('tr').children('td').children('input[name="journal_purchase_list_name[]"]').val(data.invoice_customer_description);
                $(row_journal_id).closest('tr').children('td').children('input[name="journal_purchase_list_debit[]"]').val(invoice_customer_vat_price_debit.toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,"));
                $(row_journal_id).closest('tr').children('td').children('input[name="journal_purchase_list_credit[]"]').val(invoice_customer_vat_price_credit.toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,"));
                $('.select').selectpicker();
            }
