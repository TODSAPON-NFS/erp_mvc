...
if( (int)$finance_credit['supplier_branch'] * 1 == 0){
    $branch = " สำนักงานใหญ่";
} else {
    $branch =  "สาขา " . ((int)$finance_credit['supplier_branch'] * 1) ;
} 
...
<div class="main">
    ...
    <div align="center" style="font-size:12px;">ใบจ่ายเงิน</div>
    <div align="center" style="font-size:12px;">OFFICIAL PAYMENT</div>
        ...
        <table width="100%" heigth="180">
            <tr>
                <td style="padding:4px;">
                    <div style="display:block;padding:8px;"> '.$finance_credit['finance_credit_name'].'  '.$branch.' </div>
                    <div style="display:block;padding:8px;"> '. $finance_credit['finance_credit_address'].'</div>
                    <div style="display:block;padding:8px;"> เลขประจำตัวผู้เสียภาษี / Tax : '.$finance_credit['finance_credit_tax'].' </div>
                </td>
                <td width="240">
                    <table>
                        <tr>
                            ...
                            <td width="100" height="24" valign="middle" align="left">
                               : '.$finance_credit['finance_credit_code'].'
                            </td>
                        </tr>
                        ...
                        <tr>
                            ...
                            <td width="100" height="24" valign="middle" align="left">
                            : '.$finance_credit['finance_credit_date'].'
                            </td>
    ...
    //count($tax_reports)
    for($i=$page_index * $lines; $i < count($finance_credit_lists) && $i < $page_index * $lines + $lines; $i++){ 
        ...
        $total += $finance_credit_lists[$i]['finance_credit_list_balance'];
                ...
                <tr class="odd gradeX">
                        ...
                        <td align="center">
                            '. $finance_credit_lists[$i]['invoice_supplier_code'].'
                        </td>
                        <td align="center">
                            '. $finance_credit_lists[$i]['finance_credit_list_date'].'
                        </td>
                        <td align="center">
                            '. $finance_credit_lists[$i]['finance_credit_list_due'].'
                        </td>
                        <td align="center">
                            '. $finance_credit_lists[$i]['finance_credit_list_billing'].'
                        </td>
                        <td align="right">
                            '. number_format($finance_credit_lists[$i]['finance_credit_list_amount'],2).'
                        </td>
                        <td align="right">
                            '. number_format($finance_credit_lists[$i]['finance_credit_list_balance'],2).'
                        </td>
                    </tr>
                ';
    }
    ...
    if($page_index + 1 == $page_max){
        if(count($finance_credit_lists) % $lines > 0){
            for($i = count($finance_credit_lists) % $lines ; $i < $lines; $i++){
                $html[$page_index] .= ' 
                    <tr class="odd gradeX">
                            <td align="center"> 
