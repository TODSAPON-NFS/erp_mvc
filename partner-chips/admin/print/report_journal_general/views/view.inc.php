...
$cr_total = 0;
 ...
 $row_max = count($journal_general_lists) + count($check_pays) + count($checks) + count($invoice_suppliers) + count($invoice_customers);
 ...
 while($i <  $row_max ){
    ...
    <table width="100%" border="0" cellspacing="0">
        <tr>
            <td align="left" > </td>
            <td colspan = "2" align="left" style="font-size:13px;"> GENERAL VOUCHER </td> 
        </tr>
        <tr>
            <td align="left" ></td>
            <td width="60px" align="left" ><b>เลขที่</b></td>
            <td width="120px" align="left">'.$journal_general['journal_general_code'].'</td>
        </tr>
        <tr>
            <td align="left" ></td>
            <td align="left" ><b>วันที่</b></td>
            <td align="left" >'.$journal_general['journal_general_date'].'</td>
        </tr> 
    </table>  
    <br>
    <div style="font-size:13px;"><b>สมุดรายวันทั่วไป</div>
    <div style="font-size:13px;"><b>รายละเอียด :</b> '.$journal_general['journal_general_name'].' </div>
    ...
    $dr_page = 0;
    $cr_page = 0;
    for( ;$i < count($journal_general_lists) ; $i++){
        $dr_page +=  $journal_general_lists[$i]['journal_general_list_debit'];
        $cr_page +=  $journal_general_lists[$i]['journal_general_list_credit'];
        $dr_total +=  $journal_general_lists[$i]['journal_general_list_debit'];
        $cr_total +=  $journal_general_lists[$i]['journal_general_list_credit'];

        if($journal_general_lists[$i]['journal_general_list_debit'] == 0){
            $journal_list_debit = "";
        }else{
            $journal_list_debit = number_format($journal_general_lists[$i]['journal_general_list_debit'],2);
        }

        if($journal_general_lists[$i]['journal_general_list_credit'] == 0){
            $journal_list_credit = "";
        }else{
            $journal_list_credit = number_format($journal_general_lists[$i]['journal_general_list_credit'],2);
        }

        $html[$page_index] .= ' 
        <tr>  
            <td  align="left" height="20px">'.$journal_general_lists[$i]['account_code'].'</td>
            <td  align="left">'.$journal_general_lists[$i]['account_name_th'].'</td> 
            <td  align="right" >
                '.$journal_list_debit.'
            </td>
            <td  align="right" >
    ...
    if(count($checks) > 0){
        ...
        ';
        for(; $i < count($journal_general_lists)  + count($checks) ; $i++){
            $ii = $i - count($journal_general_lists);
            $html[$page_index] .= ' 
            <tr>  
    ...
    if(count($check_pays) > 0){
        ...
        ';
        for(; $i < count($journal_general_lists)   + count($checks) + count($check_pays) ; $i++){
            $ii = $i - count($journal_general_lists) + count($checks) ;
    ...
    if(count($invoice_suppliers) > 0){
        ...
        for(; $i < count($journal_general_lists)   + count($checks) + count($check_pays) + count($invoice_suppliers) ; $i++){
            $ii = $i - count($journal_general_lists) + count($checks) + count($check_pays) ;
            $html[$page_index] .= ' 
            <tr>   
    ...
    if(count($invoice_customers) > 0){
        ...
        ';  
        for(; $i < count($journal_general_lists)  + count($checks) + count($check_pays) + count($invoice_suppliers) + count($invoice_customers) ; $i++){
            $ii = $i - count($journal_general_lists) + count($checks) + count($check_pays) + count($invoice_suppliers) ;
            $html[$page_index] .= ' 
            <tr>   
...
if($page_index == 0){
    ...
    <tfoot>
        <tr> 
            ...
            <td  align="right" style="border-top: 1px solid black;border-bottom: 1px solid black;padding:8px 0px;"  height="20px" >'.number_format($dr_page,2).'</td>
            <td  align="right" style="border-top: 1px solid black;border-bottom: 1px solid black;padding:8px 0px;" >'.number_format($cr_page,2).'</td> 
        </tr>
        <tr>
            <td colspan="4" align="center" height="20px" > </td>
...
<table width="100%">
    ...
    <tr> 
        <td colspan="5" align="left" height="20px"  >
        <br>
        <br>
