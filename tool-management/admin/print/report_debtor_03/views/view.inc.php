...
for($page_index=0 ; $page_index < $page_max ; $page_index++){
    ...
    <div align="center" style="font-size:14px;color:#00F;"> <b>รายงานใบลดหนี้/รับคืนสินค้า</b></div>
    ...
    <table  width="100%" cellspacing="0" style="font-size:12px;">
        <thead>
            <tr>
                ...
                <th colspan="3" align="center" style="border-bottom: 1px dotted black;" >ใบลดหนี้/รับคืนสินค้า</th>
                ...
                <th align="center" >มูลค่าใบกำกับเดิม</th>  
                <th align="center" >มูลค่าที่ถูกต้อง</th>  
                <th align="center" >ผลต่าง</th>  
                <th align="center" >จำนวนภาษีมูลค่าเพิ่ม</th>  
                <th align="center" >จำนวนเงินรวมทั้งสิ้น</th>  
            </tr>
            <tr> 
                <th></th>
    ...
    for($i=$page_index * $lines; $i < count($debtor_reports) && $i < $page_index * $lines + $lines; $i++){
        $branch = (int)$debtor_reports[$i]['credit_note_branch'];
        if($branch == 0){
            $branch_main = "/";
            $branch_sub = "";
        }else{
        ...
        $total_page +=  $debtor_reports[$i]['credit_note_total_price']; 
        $vat_page +=  $debtor_reports[$i]['credit_note_vat_price']; 
        $net_page +=  $debtor_reports[$i]['credit_note_net_price']; 

        $total_total +=  $debtor_reports[$i]['credit_note_total_price']; 
        $vat_total +=  $debtor_reports[$i]['credit_note_vat_price']; 
        $net_total +=  $debtor_reports[$i]['credit_note_net_price']; 

                $html[$page_index] .= ' 
                <tr>
                    <td align="center" >'.($i + 1).'</td>
                    <td align="center" >'.$debtor_reports[$i]['credit_note_date'].'</td>
                    <td>'.$debtor_reports[$i]['credit_note_code'].'</td> 
                    <td>'.$debtor_reports[$i]['credit_note_type_name'].'</td> 
                    <td>['.$debtor_reports[$i]['customer_code'].'] '.$debtor_reports[$i]['credit_note_name'].' </td>
                    <td>'.$debtor_reports[$i]['credit_note_tax'].' </td>
                    <td align="center" >' .$branch_main.'</td>
                    <td align="center" >' .$branch_sub.'</td>
                    <td  align="right" >
                        '.number_format($debtor_reports[$i]['credit_note_total_old'],2).'
                    </td> 
                    <td  align="right" >
                        '.number_format($debtor_reports[$i]['credit_note_total'],2).'
                    </td> 
                    <td  align="right" >
                        '.number_format($debtor_reports[$i]['credit_note_total_price'],2).'
                    </td> 
                    <td  align="right" >
                        '.number_format($debtor_reports[$i]['credit_note_vat_price'],2).'
                    </td> 
                    <td  align="right" >
                        '.number_format($debtor_reports[$i]['credit_note_net_price'],2).'
                    </td> 
                </tr> 
                ';
    }
    ...
    if($page_index+1 < $page_max ){
        $html[$page_index] .= ' 
                ...
                <tfoot>
                    <tr>
                        <td></td>
                        <td colspan="9" align="left"> <b>รวมแต่ละหน้า</b> </td>
        ...
        $html[$page_index] .= ' 
                ...
                <tfoot>  
                    <tr>
                        <td></td>
                        <td colspan="9" align="left"><div><b>รวมทั้งสิ้น งวด </b> '.$section_date.' (<b>เริ่มจาก</b> '.$date_start.' <b>ถึง</b> '.$date_end.') </div> </td>
