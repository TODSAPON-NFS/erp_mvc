...
$cr_total = 0;
 ...
 $row_max = count($journal_sale_lists) + count($check_pays) + count($checks) + count($invoice_suppliers) + count($invoice_customers);
 $break_page = false;
 $row_blank = 0;
 while($i <  $row_max ){
    $page_index ++;
    ...
    <table width="100%" border="0" cellspacing="0">
        <tr>
            <td align="left" > </td>
            <td colspan = "2" align="left" style="font-size:13px;"> SALE VOUCHER </td> 
        </tr>
        <tr>
            <td align="left" ></td>
            <td width="60px" align="left" ><b>เลขที่</b></td>
            <td width="120px" align="left">'.$journal_sale['journal_sale_code'].'</td>
        </tr>
        <tr>
            <td align="left" ></td>
            <td align="left" ><b>วันที่</b></td>
            <td align="left" >'.$journal_sale['journal_sale_date'].'</td>
        </tr> 
    </table>  
    <br>
    <div style="font-size:13px;"><b>สมุดรายวันขาย</div>
    <div style="font-size:13px;"><b>รายละเอียด :</b> '.$journal_sale['journal_sale_name'].' </div>
    ...
    if(count($journal_sale_lists) > 0 ){
        $journal_row = 1;
        for( ;$i < count($journal_sale_lists) ; $i++){
            $dr_page +=  $journal_sale_lists[$i]['journal_sale_list_debit'];
            $cr_page +=  $journal_sale_lists[$i]['journal_sale_list_credit'];
            $dr_total +=  $journal_sale_lists[$i]['journal_sale_list_debit'];
            $cr_total +=  $journal_sale_lists[$i]['journal_sale_list_credit'];
    
    
            if($journal_sale_lists[$i]['journal_sale_list_debit'] == 0){
                $journal_list_debit = "";
            }else{
                $journal_list_debit = number_format($journal_sale_lists[$i]['journal_sale_list_debit'],2);
            }
    
            if($journal_sale_lists[$i]['journal_sale_list_credit'] == 0){
                $journal_list_credit = "";
            }else{
                $journal_list_credit = number_format($journal_sale_lists[$i]['journal_sale_list_credit'],2);
            }
    
            $html[$page_index] .= ' 
            <tr>  
                <td  align="left" height="20px">'.$journal_sale_lists[$i]['account_code'].'</td>
                <td  align="left">'.$journal_sale_lists[$i]['account_name_th'].'</td> 
                <td  align="right" >
                    '.$journal_list_debit.'
                </td>
                <td  align="right" >
    ...
    if(count($checks) > 0){
        ...
        ';
        for(; $i < count($journal_sale_lists)  + count($checks) ; $i++){
            $ii = $i - count($journal_sale_lists) ;
            $html[$page_index] .= ' 
            <tr>  
    ...
    if(count($check_pays) > 0){
        ...
        ';
        for(; $i < count($journal_sale_lists)   + count($checks) + count($check_pays) ; $i++){
            $ii = $i - count($journal_sale_lists) - count($checks);
    ...
    if(count($invoice_suppliers) > 0){
        ...
        for(; $i < count($journal_sale_lists)   + count($checks) + count($check_pays) + count($invoice_suppliers) ; $i++){
            $ii = $i - count($journal_sale_lists) -count($checks)-count($check_pays);
            $html[$page_index] .= ' 
            <tr>   
                <td  align="left" height="20px" >#ภาษีซื้อ '.$ii.' '.$invoice_suppliers[$ii]['invoice_supplier_code'].'</td>
    ...
    if(count($invoice_customers) > 0){
        ...
        ';  
        for(; $i < count($journal_sale_lists)  + count($checks) + count($check_pays) + count($invoice_suppliers) + count($invoice_customers) ; $i++){
            $ii = $i - count($journal_sale_lists)  - count($checks)-count($check_pays)-count($invoice_suppliers);
            $html[$page_index] .= ' 
            <tr>   
